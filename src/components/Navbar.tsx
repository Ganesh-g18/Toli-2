...
const links = [
  ...
  { label: "Offers", id: "offers", icon: Tag },
  { label: "About", id: "about", icon: Info },
  { label: "Gallery", id: "gallery", icon: Image },
  { label: "Why Us", id: "why-us", icon: ShieldCheck },
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const current = links.map((link) => document.getElementById(link.id)).filter(Boolean)
  ...
  return (
    <nav className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ease-out ${open ? "bg-white backdrop-blur-2xl border-b border-border" : scrolled ? "bg-white/92 backdrop-blur-2xl border-b border-border shadow-[0_16px_42px_-28px_hsl(220_15%_20%/0.26)]" : "bg-white/78 backdrop-blur-xl border-b border-white/70"}`}>
      <div className="max-w-7xl mx-auto px-3 min-[360px]:px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        {!isMobile && <><div className="flex items-center gap-7">{links.map((link) => <button key={link.label} onClick={() => scrollTo(link.id)} className={`relative text-sm font-medium transition-colors duration-200 group ${active === link.id ? "text-primary" : "text-foreground/80 hover:text-primary"}`}>{link.label}<span className={`absolute -bottom-1 left-0 h-0.5 bg-primary transition-[width] duration-300 ease-out ${active === link.id ? "w-full" : "w-0 group-hover:w-full"}`} /></button>)}</div><a href="tel:+918074946335" className="luxury-button flex items-center gap-2 rounded-md px-2 py-1 text-sm text-foreground/80 hover:text-primary"><Phone className="w-4 h-4" />+91-8074946335</a></>}
            ...
            {links.map((link, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-300 ease-out text-left group ${
